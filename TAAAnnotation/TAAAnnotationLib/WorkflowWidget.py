...
Simplified workflow:
- Phase 1: Load Data (CT, seg mask, pre-computed centerline)
- Phase 2: Zonal Landmarks (pick zones on centerline)
- Phase 3: Export / Submit
"""
...
class WorkflowWidget(qt.QWidget):
    ...
    def _setupUI(self):
        ...
        self.btnExport = qt.QPushButton("3. Export & Reset")
    ...
    def _createZonesGroup(self, parentLayout):
        ...
        self.groupZones = qt.QGroupBox("2. Zonal Landmarks (Preview & Confirm)")
    ...
    def updateUIState(self, phase: int, orthancMode: bool = False):
        ...
        # Zone picker enabled right after data is loaded (phase >= 1)
        self.btnClickMode.setEnabled(phase >= 1)
        ...
        if orthancMode:
            ...
            self.btnExport.setEnabled(phase >= 1)
    ...
    def setOrthancMode(self, enabled: bool):
        ...
        if enabled:
            ...
            self.btnExport.setText("3. Export & Reset (Disabled - Use Orthanc)")
        else:
            self.btnExport.setToolTip("")
            self.btnExport.setStyleSheet("text-align: center; padding: 10px; font-weight: bold; background-color: #007bff; color: white;")
            self.btnExport.setText("3. Export & Reset")
